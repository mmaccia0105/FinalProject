...
write_csv(diabetes_model_data, "~/ST558 Repo/Final Project/model_data.csv")
```
...
-   Model 2: `bmi` + `exercise` + `veggies`
    ...
    -   It is known that having healthy weight and regular exercise reduces the risk of Type 2 Diabetes. In this model, we will evaulate those 2 variables. There will also be an interaction between the 2, since it could be possible that lower BMIs may exercise more. Also, adding in if vegetables are part of the diet may just indicate overall healthier individuals. We will also model an interaction with all 3 of these variables.
...
```{r}
rbind(lr_1_fit |> collect_metrics(),
...
Now we will fit a classification tree. First we will need to create the recipe, but we can use our initial recipe from above since we do not need to worry above interactions.
...
Now let's fit our model. Since we don't have to worry about interactions with random forest models, we can use the recipe from above.
...
```{r}
logloss_lr <- mn_log_loss(predict_lr, truth = diabetes, .pred_Yes) #calculating log loss
  #above needs the prediction, then the truth (diabetes column) and .pred_Yes indicated predicted probabilities
...
Overall, the classification tree was the best model since it's LogLoss was lowest at 1.715. I did not expect the classification tree to perform best, since above it was described how classification trees can overfit data. I wonder if since we only used 1 metric, the potentially overfit model performed best. If we chose more metrics, maybe we would have had a different outcome and another model would've performed better. 

```{r}
#save the model to use later

saveRDS(final_tree_diabetes, file = file.path("~", "ST558 Repo", "Final Project", "best_diabetes_model.rds"))
```
